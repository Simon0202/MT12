...
\usepackage{amsmath}
\usepackage{amsfonts}
%Pour faire un tableau de dŽrivŽ
...
\date{Printemps 2016}
  ...
  %  \begin{minipage}{0.4\textwidth}
     % \begin{flushleft} \large
        ...
        %\emph{Etablissement : } Universit\'e de Technologie de CompiÂ\`egne
      %\end{flushright}
    %\end{minipage}
\maketitle
...
% 5
  ...
  \left\{
      ...
      \end{aligned}
    \right.\]
	La fonction f est d\'efinie sur l'intervalle ($-\pi;\pi$). Elle n'admet aucune valeur interdite et on a $f(0^+)=f(0^-)=\sqrt{0}=0$ et $f(\pi/2)=0$ et $\lim\limits_{\substack{x \rightarrow \pi/2 \\ x>\pi/2}} f(x)$. Elle est continue en 0 mais pas en $\pi/2$, elle est donc continue par morceaux.
	\begin{figure}[h!]
		\centering
...
On remarque donc qu'au voisinage d'une discontinuit\'e une bosse apparait. Or quand $n$ augmente cette derni\`ere subsiste de part et d'autre de d'une discontinuit\'e. Pla\c cons nous alors ˆ droite du point de discontinuit\'e $0$ par exemple. On note que lorsque $n$ augment, le point ou il y a un pic s'approche de $0$ mais la hauteur reste constante. Nous allons essayer d'expliquer ce ph\'enom\`ene dans la partie suivante.
\end{enumerate}
\newpage
...
On note que la fonction pr\'esente une discontinuit\'e en tout point multiple de pi. La limite ˆ droite et la limite ˆ gauche en ces points vaut +1 et -1. De plus en dehors de ces points la fonctions est continue. Nous avons donc une fonction continue par morceau et sa d\'eriv\'e l'est \'egalement. Ainsi on peut dire que $f$ est $C^{1}$ par morceaux
	\subsection{Analyse Math\'ematique}
		\subsubsection{Application du Th\'eor\`eme de Dirichel}
Comme f impaire on a donc :

$$S_{f(x)}&=&\frac{4}{\pi}\sum\limits_{k=1}^{\infty}\frac{sin(2k-1)x}{2k-1}\qquad,\;\text{o\`u}\;k\,\in\,\pmb{\mathbb{R}}$$\\


Pour une fonction C1 par morceaux, le th\'eor\`eme de dirichlet nous indique quel est la limite de ces sommes partielles. Si on consid\`ere un point de continuit\'e de la fonction, ces sommes partielles vont converger vers f(x).  Ainsi au point de discontinuit\'e ($x=0$), la limite de ces sommes partielles va \^etre la demi somme limite \`a droite, limite \`a gauche de la valeur de la fonction en le point consid\'erer\'e. NŽanmoins ici la demi somme vaut $0$ :\\


$$S_{f(x)}&=&\frac{4}{\pi}\sum\limits_{k=1}^{\infty}\frac{sin(2k-1)x}{2k-1} \vert{_{x=0}} = \frac{1}{2}[+1+(-1)] = 0$$\\

En particulier, sur un tel point la limite ne vaut pas la valeur du signal $f(0)=1$ alors que la demi somme vaut $0$

On s'int\'eresse par la suite au comportement au voisinage du point

		\subsubsection{Comportement au voisinage d'un point de discontinuit\'e}

\'Etudions la somme partielle $S_{p}(x)$:\\
Calculons donc sa d\'eriv\'ee afin d'\'etablir son tableaux de variations. \\
Comme $\frac{d}{dt}sin((2k-1)x) = (2k-1)cos((2k-1)x)$ on d\'eduit que \\
$$\frac{d}{dt}S_{p}(x) = \frac{4}{\pi}\sum\limits_{k=1}^{\infty}cos((2k-1)x)$$\\
Ensuite on pose : \\
\begin{itemize}
	\item $cos((2k-1)x) = Re [\exp(i(2k-1)x)]$\\
	
	\item $\sum\limits_{k=0}^{n}q^{k} = \frac{1-q^{n+1}}{1-q}$\\
\end{itemize} 
...
\left \{
  \begin{tabular}{c}
  $\frac{d}{dt}S_{p}(x)= \frac{2}{\pi}\frac{sin(2kx)}{sin(x)}$ pour $t \neq 0$\\
  $\frac{d}{dt}S_{p}(x)= \frac{4}{\pi}$ par calcul direct\\
  \end{tabular}
\]
...
Soit $x \in [0,\pi]$ tels que $S_{p}^{'}(x) = \frac{d}{dt}S_{p}(x) = \frac{2}{\pi}\frac{sin(2pu)}{sin(u)} = 0$. On obtient les z\'eros de la d\'eriv\'e suivants : \\
\left \{
  \begin{tabular}{c}
  $sin(2px)=0 \iff 2pt = 0$ modulo $\pi$\\
  \end{tabular}
\]\newpage

Donc pour $x \in ]0,\pi[  $S_{p}^{'}(x) = 0 \iff x \in \{\frac{\pi}{2p},\frac{2\pi}{2p},...,{\frac{(2p-1)\pi}{2p}\}$\\
...
$$m_{1,p} = \frac{2}{\pi}\int_{0}^{\pi}\frac{sin v}{v}\frac{1}{\frac{sin(\frac{v}{2p})}{\frac{v}{2p}}} \frac{dv}{}$$\\
...
$$\lim\limits_{p \rightarrow +\infty} m_{1,p} = \frac{2}{\pi} \lim\limits_{p \rightarrow +\infty} \int_{0}^{\pi}\frac{sin v}{v}\frac{1}{\frac{sin(\frac{v}{2p})}{\frac{v}{2p}}} \frac{dv}{} $$\\

On fait rentrer la limite ˆ l'intŽrieur de l'intŽgrale car $f$ est $C^{1}$ :\\
 $$\frac{2}{\pi} \lim\limits_{p \rightarrow +\infty} \int_{0}^{\pi}\frac{sin v}{v}\frac{1}{\frac{sin(\frac{v}{2p})}{\frac{v}{2p}}} \frac{dv}{} = \frac{2}{\pi} \int_{0}^{\pi} \lim\limits_{p \rightarrow +\infty} \bigg[\frac{sin v}{v}\frac{1}{\frac{sin(\frac{v}{2p})}{\frac{v}{2p}}}\bigg] \frac{dv}{} $$\\
 D'aprs les points \ref{}\ref{} on a le rŽsultat suivant :
 $$\frac{2}{\pi} \int_{0}^{\pi} \lim\limits_{p \rightarrow +\infty} \bigg[\frac{sin v}{v}\frac{1}{\frac{sin(\frac{v}{2p})}{\frac{v}{2p}}}\bigg] \frac{dv}{} = \frac{2}{\pi} \int_{0}^{\pi}\frac{sin v}{v}dv $$

		\subsubsection{Conclusion}
A partir de ce point nous pouvons calculer numŽriquement le rŽsultat suivant :
$$\frac{2}{\pi} \int_{0}^{\pi}\frac{sin v}{v}dv = 1.18 = 1 + 0.18$$\\
La bosse dŽpasse donc toujours la valeur 1 de la fonction au voisinage de 0 d'une quantitŽ incompressible 0.18. L'amplitude de la bosse est donc toujours supŽrieur de 0.18. pour notre fonction.\\
La gŽnŽralisation de notre fonction (non abordŽ ici) montre que la bosse de Gibbs ˆ une hauteur proportionnelle ˆ une amplitude du saut de discontinuitŽ. Avec un coefficient de proportionnalitŽ qui est universel. Il est appelŽ le coefficient de Wilbraham Gibbs est vaut 9\% du saut.
...
\begin{quote}
Rien de sert de courir, il faut partir Ã  point.
\end{quote}
